...
import { Check, Copy, Loader2, Sparkles, RotateCcw, Bot, Gem, Save, X, Heart, List } from 'lucide-react';
...
export function ResultDisplay({
  ...
  const [showSavedPromptsPanel, setShowSavedPromptsPanel] = useState(false);
  ...
  const handleGenerateImage = async (provider: 'chatgpt' | 'gemini') => {
    ...
    try {
      ...
      if (normalized.displayUrl && normalized.editUrl) {
        const generatedBrand = metadata?.brand || 'No Brand';
        onAddGeneratedImage?.(provider, { displayUrl: normalized.displayUrl, editUrl: normalized.editUrl, referenceLabel: getReferenceLabel(), generatedBrand });
      } else {
  ...
  return (
    ...
    >
          ...
          <div className="mt-4 flex flex-wrap justify-end gap-2">
            ...
            >
              {isRegeneratingPrompt ? (
                ...
                <><Sparkles className="w-4 h-4" />Regenerate Prompt</>
              )}
            </Button>
          </div>
        </motion.div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-primary/20 bg-gradient-to-r from-primary/5 to-accent/5">
            ...
            <TooltipProvider>
              <div className="flex items-center gap-0.5 bg-background rounded-lg p-1 border border-border shadow-sm">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={() => setShowSaveModal(true)} className="h-8 w-8 text-primary hover:bg-primary/10">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={() => setShowSavedPromptsPanel(true)} className="h-8 w-8 text-primary hover:bg-primary/10">
                      <List className="w-4 h-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent><p>Saved Prompts</p></TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={onOpenFavorites} className="h-8 w-8 text-primary hover:bg-primary/10">
      ...
      {/* Saved Prompts Panel */}
      <SavedPromptsPanel
        isOpen={showSavedPromptsPanel}
        onClose={() => setShowSavedPromptsPanel(false)}
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center">
          <Button onClick={() => handleGenerateImage('chatgpt')} disabled={generatingImage.chatgpt} variant="outline" className="gap-2 w-full sm:w-auto sm:min-w-[120px]">
            {generatingImage.chatgpt ? <><Loader2 className="w-4 h-4 animate-spin" /><span className="tabular-nums">{chatgptTimer.elapsedTime}s</span></> : <><Bot className="w-4 h-4" />ChatGPT</>}
          </Button>
          <Button onClick={() => handleGenerateImage('gemini')} disabled={generatingImage.gemini} variant="outline" className="gap-2 w-full sm:w-auto sm:min-w-[120px]">
            {generatingImage.gemini ? <><Loader2 className="w-4 h-4 animate-spin" /><span className="tabular-nums">{geminiTimer.elapsedTime}s</span></> : <><Gem className="w-4 h-4" />Gemini</>}
          </Button>
          <Button onClick={handleGenerateBoth} disabled={generatingImage.chatgpt || generatingImage.gemini} variant="default" className="gap-2 gradient-primary w-full sm:w-auto sm:min-w-[140px]">
            {(generatingImage.chatgpt && generatingImage.gemini) ? <><Loader2 className="w-4 h-4 animate-spin" />Generating...</> : <><Sparkles className="w-4 h-4" />Generate Both</>}
          </Button>
        </div>
                ...
                <div key={label}>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3 sm:grid-cols-3 md:grid-cols-4">
                    {images.map((img, index) => {
                      ...
                      return (
                      <div key={`${label}-${img.provider}-${index}`} className="relative group cursor-pointer aspect-square" onClick={() => setModalImage({ displayUrl: img.displayUrl, editUrl: img.editUrl, provider: img.provider, imageId })}>
                        <div className="absolute inset-0 bg-primary/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10"><span className="text-primary-foreground bg-primary/80 px-2 py-1 rounded text-xs font-medium">View</span></div>
                        <button onClick={(e) => { e.stopPropagation(); onRemoveGeneratedImage?.(img.provider, img.originalIndex); }} className="absolute top-1 left-1 z-20 bg-destructive/90 hover:bg-destructive text-destructive-foreground rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity" title="Remove image"><X className="w-3 h-3" /></button>
                        <FavoriteHeart imageId={imageId} liked={favorites.has(imageId)} onToggle={handleToggleFavorite} className="top-1 right-8" />
                        <div className="absolute top-1 right-1 z-10">{img.provider === 'chatgpt' ? (<div className="bg-background/80 backdrop-blur-sm rounded p-1"><Bot className="w-3 h-3 text-muted-foreground" /></div>) : (<div className="bg-background/80 backdrop-blur-sm rounded p-1"><Gem className="w-3 h-3 text-muted-foreground" /></div>)}</div>
                        {img.generatedBrand && (<div className="absolute bottom-1 left-1 z-10"><span className="bg-background/80 backdrop-blur-sm text-foreground text-[10px] font-medium px-1.5 py-0.5 rounded">{img.generatedBrand}</span></div>)}
                        <img src={img.displayUrl} alt={`${label} - ${img.provider}`} className="w-full h-full object-cover rounded-lg border border-border shadow-sm" />
                      </div>
                      );
                    })}
                  </div>
